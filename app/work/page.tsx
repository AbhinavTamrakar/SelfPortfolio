...
export default function Work() {
  return (
    <section
      id="work"
      className= "{scroll-mt-16 border-b border-(--border) px-5 pt-16 bg-gray-500 text-gray-600} ${inter.className}"
    >
      <div className="mb-[clamp(2rem,6vw,4rem)] flex flex-row items-end justify-between gap-4">
        <div className="flex flex-col gap-2">
          <span className="pt-15 text-[clamp(9px,2vw,11px)] tracking-[0.08em] uppercase text-(--muted)">
            {projects.length} projects
          </span>
          <h2 className="font-['Lora',serif] text-[clamp(1.8rem,5vw,3.5rem)] font-normal tracking-[-0.02em] leading-none">
            Selected Work
          </h2>
        </div>
      </div>
...
function ProjectRow({ project }: { project: (typeof projects)[0] }) {
  return (
    <a
      href={project.link}
      className="project-row grid grid-cols-[auto_1fr_auto] gap-[clamp(1rem,4vw,2rem)] border-t border-(--border) px-0 py-[clamp(1rem,4vw,2rem)] no-underline transition-all duration-200 hover:mx-[clamp(-0.5rem,-2vw,-1.5rem)] hover:bg-black/1.5 hover:px-[clamp(0.5rem,2vw,1.5rem)]"
      style={{ color: "inherit", cursor: "pointer" }}
    >
      <span className="pt-[0.15rem] text-[clamp(9px,2vw,11px)] text-(--muted)">
        {project.number}
      </span>
      <div>
        <div className="mb-2 flex flex-wrap items-baseline gap-[clamp(0.75rem,3vw,1.5rem)]">
          ...
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="border border-(--border) px-2 py-[0.15rem] text-[clamp(8px,1.5vw,10px)] tracking-[0.06em] uppercase text-(--muted)"
              >
                {tag}
              </span>
            ))}
        ...
        <p className="m-0 max-w-140 text-[clamp(12px,2vw,13px)] text-(--muted)">
          {project.description}
        </p>
      </div>
      <div className="text-right">
        <span className="text-[clamp(9px,2vw,11px)] text-(--muted)]">{project.year}</span>
        <div className="mt-1 text-[clamp(1rem,2vw,1.2rem)] text-(--muted)">↗</div>
      </div>
    </a>
  );
}